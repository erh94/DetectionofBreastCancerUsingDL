...
def createTrainFrame(homedir,timer):
...
def createTestFrame(homedir,timer):
    timer.update(5)
...
widgets = ['Progress for data cleaning: ', pb.Percentage(), ' ', 
            pb.Bar(marker=pb.RotatingMarker()), ' ', pb.ETA()]
timer = pb.ProgressBar(widgets=widgets, maxval=10).start()
train = createTrainFrame('/media/erh/BEFCFF6AFCFF1B7B/dataset/',timer)
test = createTestFrame('/media/erh/BEFCFF6AFCFF1B7B/dataset/',timer)
timer.finish()
